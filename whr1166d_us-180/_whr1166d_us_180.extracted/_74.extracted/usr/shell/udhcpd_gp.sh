...
UDHCPD_BIN=udhcpd
UDHCPD_PID_FILE=/tmp/udhcpd_gp.pid
UDHCPD_LEASE_FILE=/tmp/udhcpd_gp.leases
UDHCPD_CONF_FILE=/tmp/udhcpd_gp.conf
UDHCPD_INTERFACE=br1
...
lan_ip=`nvram get GuestPortIP`
LanNetmask=`nvram get GuestPortMask`
DHCPSIPStart=`nvram get GuestPortDHCPStart`
DHCPSIPEnd=`nvram get GuestPortDHCPEnd`
DHCPSLeaseTime=`nvram get GuestPortDHCPLeaseTime`
...
#	fi
		echo "not_lease_ip	$lan_ip" >> $UDHCPD_CONF_FILE
...
start () {
	...
	#rm -rf /tmp/dhcpsTable
		...
		($UDHCPD_BIN $UDHCPD_CONF_FILE $UDHCPD_PID_FILE &)
	#fi
